        ...
        public async Task<AuthenticationTicket> ValidateTicket(HttpContext context, AuthenticationProperties properties, AuthenticationScheme scheme, CasOptions options, string ticket, string service)
        {
            ...
            switch (options.AuthenticationType)
            {
                case AthenticationType.Windows:
                    validateUrl = $"{validateEndpoint}?iwa=true&service={service}&ticket={Uri.EscapeDataString(ticket)}";
                    break;
                default:
                    validateUrl = $"{validateEndpoint}?service={service}&ticket={Uri.EscapeDataString(ticket)}";
                    break;
            }
